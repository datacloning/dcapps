## Bernoulli model, Bimodal Normal prior

bimodal_prior <-
function(p = 0.3, n = 10,
mu1 = -2, var1 = 1,
mu2 = 1, var2 = 2,
scale = c("prob", "logit"), seed = 0)
{
    ...
    fPri <- function(p, mu1, var1, mu2, var2)
        0.5 * (dnorm(qlogis(p), mu1, sqrt(var1)) +
            dnorm(qlogis(p), mu2, sqrt(var2)))
    Pri <- sapply(pval, fPri, mu1, var1,
        mu2, var2)
    if (scale == "prob") {
        p <- p
    ...
    matplot(pval, M, type = "l",
        ...
        sub=paste0("Mean = ", round(mean(y[1:n]), 2), " (",
            sum(1-y[1:n]), "x 0s & ", sum(y[1:n]), "x 1s)"),
